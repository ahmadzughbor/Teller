<template>
    ...
    <div class="grid gap-3 grid-cols-12">
        ...
        <!-- Question Mark -->
        <div class="mt-3 col-span-3">
            <label for="question_type" class="block text-sm font-medium text-gray-700">علامة السؤال</label>
            <Field    :name="`questions[`+index+`].mark`"
                      type="number" placeholder="علامة السؤال"
                      v-model.number="model.mark"
                      @change="dataChange"
                      class="keyword-input with-border"
                      :class="{'invalid-input':childErrors[`questions[`+index+`].mark`]??''}"
            />
            <ErrorMessage :name="`questions[`+index+`].mark`" class="error-message" />
        </div>
        <!--/ Question Mark -->
    </div>
        ...
        <div v-if="shouldHaveOptions()" class="my-2 py-2">
          ...
          "
            ...
            >
                ...
                <div class="w-2/12 mr-2" :key="option.uuid" v-if="['radio','select'].includes(model.type)">
                    <label :for="option.uuid">
                        <input :id="option.uuid" class="inline-block" type="radio" :name="'group_'+model.id" :checked="option.is_correct" value="true" v-model="option.is_correct"   @change="changeCorrectAnswer($event,indx)"   />
                        الاجابة الصحيحة
                    </label>
                </div>
                <div class="w-2/12 mr-2" :key="option.uuid" v-if="['checkbox'].includes(model.type)">
                    <label :for="option.uuid">
                        <input :id="option.uuid" class="inline-block w-4 h-4"  type="checkbox" :name="'group_'+model.id" value="true"   v-model="option.is_correct"  :checked="option.is_correct"  @change="changeCorrectAnswer($event,indx)"  />
                        الاجابة الصحيحة
                    </label>
                </div>
                <!-- Delete Option -->
...
function addOption() {
    setOptions([
        ...getOptions(),
        { uuid: uuidv4(), text: "",is_correct: false },
    ]);
    dataChange();
}
...
function changeCorrectAnswer(event,index){
    const data = model.value;
    console.log(event.target.value);
    console.log(index);

    if (['radio', 'select'].includes(data.type)) {
        data.data.options.forEach((item, idx) => {
            if (idx === index) {
                item.is_correct = Boolean(true);
            } else {
                item.is_correct = Boolean(false);
            }
        });
    }
    ...
    //     });
    //     data.data.options[index].is_correct=Boolean(event.target.value);
    // }
    //
    if(['checkbox'].includes(data.type)){
        data.data.options[index].is_correct=Boolean(event.target.value);
    }
